...
var dispositions = []Disposition{
	...
	{
		Provider: "github", Status: dispositionShipped,
		AccessPath: "provider CLI handover (Copilot CLI, `copilot -p … --output-format json`); GitHub Models — the API path — was retired 2026-07-30",
		Billing:    "subscription: one premium request per turn on Free (observed), token counts per model call in the stream",
		Capabilities: Capabilities{ChatCompletions: false, Streaming: true, Tools: true, ModelsList: false,
			ReasoningVocab:  []string{"none", "minimal", "low", "medium", "high", "xhigh", "max"},
			ReasoningByRung: map[string]string{"low": "low", "medium": "medium", "high": "high", "max": "xhigh", "ultra": "max"}},
		...
		Evidence: []string{
			...
			"live, 2026-09-06, CLI 1.0.82, Free plan, the owner's own `copilot login`: `--output-format json` streams JSONL (assistant.message_delta/message, tool.execution_start/complete, model.model_call_success usage, result with sessionId and exitCode); `--resume <id>` continues a session; `--model` other than auto is refused on Free; `--effort` is refused on auto and its dial is none…max; without --allow-all-tools a tool is denied ('could not request permission from user') while the model still answers and the exit code is 0; auto picked gpt-5.6-luna and mai-code-1.1-flash",
		},
		Blockers: []string{
			"paid Copilot plans not yet seen through a login: their rows carry the Free plan's shape as seeds",
		},
	},
